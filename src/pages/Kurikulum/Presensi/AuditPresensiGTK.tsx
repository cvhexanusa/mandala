...
const AuditPresensiGTK: React.FC = () => {
  ...
  const handleExport = () => {
    Swal.fire({
      ...
      text: "Data audit GTK yang tidak hadir akan diexport ke format CSV (Kompatibel dengan Excel).",
      icon: "question",
      ...
      if (result.isConfirmed) {
        const headers = ["No", "Nama Pegawai", "NUPTK / NIP", "Jabatan / Jenis PTK", "Jam Masuk", "Jam Pulang", "Status", "Keterangan Audit Log"];
        ...
        const filename = `Audit_Presensi_GTK_${school?.nama.replace(/\s+/g, "_") || "Sekolah"}_${selectedDate}.csv`;
